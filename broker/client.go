...
import (
	...
	"github.com/mafanr/g"
	"github.com/mafanr/meq/internal/message"
	"github.com/mafanr/meq/internal/network/mqtt"
...
func newClient(id uint64, conn net.Conn, b *Broker) *client {
	return &client{
		id:        id,
		conn:      conn,
		bk:        b,
		initTime:  time.Now(),
		msgSender: make(chan []*message.PubMsg, MAX_CHANNEL_LEN),
...
func (c *client) start() {
	...
	if err != nil {
		fmt.Println("cant receive connect packet from client", err, zap.Uint64("cid", c.id))
		return
	}

	g.L.Info("new user online", zap.Uint64("cid", c.id), zap.String("username", string(c.un)), zap.String("ip", c.conn.RemoteAddr().String()))

	go c.writeLoop()
	err = c.readLoop()
	if err != nil {
		if !talent.IsEOF(err) {
			g.L.Info("client read loop error", zap.Uint64("cid", c.id), zap.Error(err))
		}
	}
}
...
func (c *client) readLoop() error {
	defer func() {
		g.L.Info("user offline,close read", zap.Uint64("conn_id", c.id))
		c.closed = true
		c.closech <- struct{}{}
		// unsub topics
...
func (c *client) writeLoop() {
	defer func() {
		g.L.Info("user offline,close write", zap.Uint64("conn_id", c.id))
		c.closed = true
		c.conn.Close()
		if err := recover(); err != nil {
