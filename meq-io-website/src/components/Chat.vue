<template>
  <div class="chat" style="min-height:170px">
        <h2>Play a chatting demo</h2>
        <Row class="margin-top-20">
             <Col v-show="!userIsSet" :xs="{span:20,offset:0}" :sm="{span:14,offset:0}" :md="{span: 14,offset:0}" :lg="{ span: 14, offset: 0 }">
                <input class="border-radius-small bg-transparent font-color-white" style="width:60%;padding:4px 10px;" type="text" v-model="username" placeholder="please input your username" @keyup.enter="setUser">
                <Button @click="setUser" type="info" class="margin-left-10" size="large">Join chat!</Button>
            </Col>
            <Col span="17" offset="1" v-show="userIsSet">
              ...
              <div style="text-align:center;font-size:13px"><Button size='small' type="primary" @click="loadMore">Load more</Button></div>
              <div  id="chat-room" style="overflow-y:scroll;max-height:400px;">
                  <Row v-for="msg in messages" :key="msg.id" class="animated bounceInRight margin-top-10">
                    <div>
                      <div style="font-size:15px;color:#777">{{msg.sender}} <span style="float:right;font-size:13px;">{{msg.timestamp}}</span></div>
                      <span :class="{'bg-green':msg.sender==username}" style="padding:3px 5px;margin-top:10px">{{msg.payload.toString()}}</span>
                    </div>
                </Row>
              </div>
              ...
              <Row style="margin-top:20px">
                  <div  class="input-group">
                      <input class="border-radius-small bg-transparent font-color-white form-control"  style="width:80%;padding:3px 10px;" type="text"  v-model="message" placeholder="Say Something.." @keyup.enter="sendMessage()">
                      <span  style="width:20%;" class="input-group-btn">
                          <Button class="btn btn-default form-control margin-left-10" type="info" size="large" v-on:click="sendMessage()">Say it! <i class="fa fa-commenting-o"></i></Button>
                      </span>
                  </div> 
                  <ul class="emoji">
            ...
            <Col span="4" offset="2" v-show="userIsSet" style="overflow-y:scroll;max-height:600px;">
              <div style="margin-bottom:5px;">online users</div>
              <Row v-for="u in users"  class="animated bounceInLeft margin-left-10">
...
export default {
  ...
  methods: {
    ...
    setUser() {
        if (this.username == '') {
            this.$Message.error("username cant be empty")
            return 
        }
      this.userIsSet = true
       var _this = this;
